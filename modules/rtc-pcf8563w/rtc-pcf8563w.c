 ...
 * Copyright 2023 Seeed Technology Inc.
 *
 * Author: Alessandro Zummo <a.zummo@towertech.it>
 * Maintainers: http://www.nslu2-linux.org/
 *
 * Change log:
 * 2020-03-31: Modified by Seeed Technology Inc. Add support for watchdog
 *
...
/* watchdog driver */
static int pcf8563_wdt_ping(struct watchdog_device *wdd)
{
    struct pcf8563 *pcf8563 = watchdog_get_drvdata(wdd);
...
static int pcf8563_wdt_active_ping(struct watchdog_device *wdd)
{
    int ret = 0;

    if (watchdog_active(wdd))
    {
        ret = pcf8563_wdt_ping(wdd);
        if (ret)
            dev_err(wdd->parent,
...
static int pcf8563_wdt_start(struct watchdog_device *wdd)
{
    struct pcf8563 *pcf8563 = watchdog_get_drvdata(wdd);
    struct i2c_client *client = pcf8563->client;
    int err;
    ...
    dev_dbg(wdd->parent, "%s: start watchdog", __func__);

    /* set watchdog timer value */
    err = pcf8563_wdt_ping(wdd);
    if (err)
        return err;

    /* enable watchdog timer */
    val = PCF8563_TMRC_ENABLE | (pcf8563->freq & PCF8563_TMRC_MASK);
    err = pcf8563_write_block_data(client, PCF8563_REG_TMRC, 1, &val);

    return err;
}

static int pcf8563_wdt_stop(struct watchdog_device *wdd)
{
    struct pcf8563 *pcf8563 = watchdog_get_drvdata(wdd);
    struct i2c_client *client = pcf8563->client;
    unsigned char val;
    ...
    return pcf8563_write_block_data(client, PCF8563_REG_TMRC, 1, &val);
}

static int pcf8563_wdt_set_timeout(struct watchdog_device *wdd,
                                   unsigned int new_timeout)
{
    dev_dbg(wdd->parent, "%s: new watchdog timeout: %is (old: %is)\n",
            __func__, new_timeout, wdd->timeout);

    wdd->timeout = new_timeout;

    return pcf8563_wdt_active_ping(wdd);
}

static const struct watchdog_info pcf8563_wdt_info = {
    .identity = "NXP PCF8563 Watchdog",
    .options = WDIOF_KEEPALIVEPING | WDIOF_SETTIMEOUT,
};

static const struct watchdog_ops pcf8563_watchdog_ops = {
    .owner = THIS_MODULE,
    .start = pcf8563_wdt_start,
    .stop = pcf8563_wdt_stop,
    .ping = pcf8563_wdt_ping,
    .set_timeout = pcf8563_wdt_set_timeout,
};

static int pcf8563_watchdog_init(struct device *dev, struct pcf8563 *pcf8563)
{
    int err;
    unsigned char val;
    u32 timer_freq;
    ...
    pcf8563->freq = PCF8563_TMRC_1;

    pcf8563->wdd.parent = dev;
    pcf8563->wdd.info = &pcf8563_wdt_info;
    pcf8563->wdd.ops = &pcf8563_watchdog_ops;
    pcf8563->wdd.min_timeout = PCF8563_WD_VAL_MIN;
    pcf8563->wdd.max_timeout = PCF8563_WD_VAL_MAX;
    pcf8563->wdd.timeout = PCF8563_WD_VAL_DEFAULT;
    pcf8563->wdd.min_hw_heartbeat_ms = PCF8563_WD_VAL_MIN * 1000;

    of_property_read_u32(dev->of_node, "timer-frequency", &timer_freq);
    of_property_read_u32(dev->of_node, "default-timeout", &pcf8563->wdd.timeout);
    of_property_read_u32(dev->of_node, "min-hw-heartbeat-ms", &pcf8563->wdd.min_hw_heartbeat_ms);

    if (timer_freq < PCF8563_TMRC_4096)
    {
        timer_freq = PCF8563_TMRC_4096;
    }
    else if (timer_freq > PCF8563_TMRC_1_60)
    {
        timer_freq = PCF8563_TMRC_1_60;
    }

    pcf8563->freq = timer_freq;

    if (pcf8563->wdd.timeout < PCF8563_WD_VAL_MIN)
    {
        pcf8563->wdd.timeout = PCF8563_WD_VAL_MIN;
    }
    else if (pcf8563->wdd.timeout > PCF8563_WD_VAL_MAX)
    {
        pcf8563->wdd.timeout = PCF8563_WD_VAL_MAX;
    }
...
static int pcf8563_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
    ...
    pcf8563->c_polarity = (buf[PCF8563_REG_MO] & PCF8563_MO_C) ? (tm->tm_year >= 100) : (tm->tm_year < 100);
