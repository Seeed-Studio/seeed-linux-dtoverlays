...
/*
 * work_cb_irq: scan mpr121's interrupt.
 */
static void work_cb_irq(struct work_struct *work)
{
	struct mpr121_touchkey *mpr121 = container_of(work, struct mpr121_touchkey, work_irq);
			      ...
			      const struct i2c_device_id *id)
{
	...
	if(client->irq){
		...
		INIT_WORK(&mpr121->work_irq, work_cb_irq);
		mpr121->exit_work = false;
		schedule_work(&mpr121->work_irq);
	}
...
static int mpr_touchkey_remove(struct i2c_client *client)
{
	...
	if (!client->irq) {
		mpr121->exit_work = true;
		cancel_work_sync(&mpr121->work_irq);
	}else
